...
set -uo pipefail
cd "$(dirname "$0")"
OUT="/tmp/keccak-ab-$(date +%H%M%S).log"; echo "$OUT" > /tmp/keccak-ab-logpath.txt
export TMPDIR="/tmp/mpt-keccak-ab-$$"; mkdir -p "$TMPDIR"
...
{
  echo "##### OLD (vec! per hash) #####"
  if git stash push -m keccak-ab -- benches/large.rs examples/reopen.rs src/lib.rs; then STASHED=1; fi
  cargo build --release --bench large 2>&1 | tail -1
  run
  echo "##### NEW (streaming keccak) #####"
