...
- **Widen `DiskPtr` now.** `unit: u32` × 256 B = 1 TiB ceiling already panicked at ~70M
  accounts under bulk-load churn; tip-scale GC high-water will cross it. u64 unit (or
  larger `ADDR_UNIT`), respecting the `RamChild <= 56 B` frontier guard.
